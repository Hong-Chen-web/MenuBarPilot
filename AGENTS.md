...
MenuBarPilot is a SwiftUI + AppKit hybrid macOS menu bar app with two features: menu bar icon management and Codex session monitoring. `AppState.shared` (singleton `@MainActor ObservableObject`) is the single source of truth; all views bind via `@EnvironmentObject`.
...
**Key data flows:**
- Codex session paths use an "encoded path" format: `/` → `-` (e.g. `/Users/foo/AI` becomes `-Users-foo-AI`), so JSONL files live at `~/.Codex/projects/-Users-foo-AI/<sessionId>.jsonl`.
